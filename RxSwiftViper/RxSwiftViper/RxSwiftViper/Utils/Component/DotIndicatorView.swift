...
public class DotIndicatorView: UIView {
    ...
    private var info: UILabel = {
        let label: UILabel = Commons.label(id: "loading", type: .regular, text: "Please wait...", size: 13, addColor: .textGray, align: .center)
        return label
    }()
    private var views: UIView = {
        let view: UIView = Commons.view(addColor: .white)
        view.roundedView(cornerRadius: 15, bgColor: .white, isShadow: true)
        return view
    }()
    private var loadingviews: UIView = {
        let view: UIView = Commons.view(addColor: .clear)
