...
class Utils {
    ...
    static func groupedSectionsByDate(from busesSchedule: [BusScheduleModel]) -> [BusScheduleSection] {
        let grouped = Dictionary(grouping: busesSchedule) { $0.datetime?.dateString ?? "30/06/2021" }
        let sections = grouped.map { BusScheduleSection(date:  $0.key, buses: $0.value) }
        return sections
    }
}
